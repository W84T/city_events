@php
    use App\Models\Association;use Filament\Support\Enums\Alignment;
    ...
    use Illuminate\Contracts\Pagination\CursorPaginator;use Illuminate\Contracts\Pagination\LengthAwarePaginator;use Illuminate\Contracts\Pagination\Paginator;use Illuminate\Support\Arr;use Illuminate\Support\Js;use Illuminate\Support\Str;use function Filament\Support\generate_href_html;use function Filament\Support\prepare_inherited_attributes;
                                ...
                                >
                                    ...
                                    >
                                        <div class="flex-1">
                                            @if ($recordUrl)
                                                <a
                                                    {{ generate_href_html($recordUrl, $openRecordUrlInNewTab) }}
                        ...
                        <x-filament-tables::row>
                            ...
                            @foreach ($columns as $column)
                                ...
                                <x-filament-tables::cell
                                    @class([
                                        'fi-table-individual-search-cell-' . str($columnName)->camel()->kebab(),
                                        'min-w-48 px-3 py-2' => $isIndividuallySearchable,
                                    ])
                                >
                                    @if ($isIndividuallySearchable)
                                        @switch($columnName)
                                            @case('exhibition.name')
                                                @php
                                                    $options = Association::where('type', 'exhibition')->pluck('name', 'name')->toArray();
                                                @endphp

                                                <x-filament-tables::select-search-field
                                                    :debounce="$searchDebounce"
                                                    :on-blur="$isSearchOnBlur"
                                                    wireModel="tableColumnSearches.{{ $columnName }}"
                                                    :options="$options"
                                                />
                                                @break
